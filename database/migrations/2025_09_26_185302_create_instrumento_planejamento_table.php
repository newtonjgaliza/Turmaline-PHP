<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        if (!Schema::hasTable('instrumento_planejamento')) {
            // Cria a tabela se não existir
            Schema::create('instrumento_planejamento', function (Blueprint $table) {
                $table->id(); // SERIAL PRIMARY KEY

                $table->boolean('multiyearplan')->nullable();
                $table->integer('points_multiyearplan')->nullable();

                $table->boolean('budgetguidelineslaw')->nullable();
                $table->integer('points_budgetguidelineslaw')->nullable();

                $table->boolean('annualbudgetlaw')->nullable();
                $table->integer('points_annualbudgetlaw')->nullable();

                $table->unsignedBigInteger('municipio_id')->nullable(); // FK
                $table->timestamps();

                $table->foreign('municipio_id')
                      ->references('id')
                      ->on('municipios') // no plural
                      ->onDelete('cascade')
                      ->onUpdate('cascade');
            });
        } else {
            // Se já existir, adiciona colunas que faltarem
            Schema::table('instrumento_planejamento', function (Blueprint $table) {
                $columns = Schema::getColumnListing('instrumento_planejamento');

                $campos = [
                    'multiyearplan',
                    'budgetguidelineslaw',
                    'annualbudgetlaw',
                    'municipio_id'
                ];

                foreach ($campos as $campo) {
                    if (!in_array($campo, $columns)) {
                        if ($campo === 'municipio_id') {
                            $table->unsignedBigInteger('municipio_id')->nullable();
                            $table->foreign('municipio_id')
                                  ->references('id')
                                  ->on('municipios')
                                  ->onDelete('cascade')
                                  ->onUpdate('cascade');
                        } else {
                            $table->boolean($campo)->nullable();
                            $table->integer("points_{$campo}")->nullable();
                        }
                    } else {
                        // Se o campo já existe mas o points não, adiciona apenas o points
                        $pointsCampo = "points_{$campo}";
                        if (!in_array($pointsCampo, $columns)) {
                            $table->integer($pointsCampo)->nullable();
                        }
                    }
                }
            });
        }
    }

    public function down(): void
    {
        Schema::dropIfExists('instrumento_planejamento');
    }
};
